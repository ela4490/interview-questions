# Difference between submit and execute : 
http://javarevisited.blogspot.in/2016/04/difference-between-ExecutorServie-submit-vs-Executor-execute-method-in-Java.html#more 

# Difference between Executer Framework and ForkJoinPool : 
http://javarevisited.blogspot.in/2016/12/difference-between-executor-framework-and-ForkJoinPool-in-Java.html#more 

# Future and FutureTask : 
http://javarevisited.blogspot.in/2015/01/how-to-use-future-and-futuretask-in-Java.html#axzz4zBGrCJAG 

# How to stop a thread : 
http://www.java67.com/2015/07/how-to-stop-thread-in-java-example.html 

# Difference between Thread and Executor : 
http://javarevisited.blogspot.in/2016/12/difference-between-thread-and-executor.html 

# Java concurrency interview ques : 
http://www.journaldev.com/1162/java-multi-threading-concurrency-interview-questions-with-answers 

# Lock interface : 
http://www.journaldev.com/2377/java-lock-example-and-concurrency-lock-vs-synchronized 

# 50 interview questions : 
http://javarevisited.blogspot.in/2014/07/top-50-java-multithreading-interview-questions-answers.html 

# Avoid race condition between wait and notify : 
http://javarevisited.blogspot.sg/2011/05/wait-notify-and-notifyall-in-java.html 

# Daemon threads : 
http://javarevisited.blogspot.sg/2012/03/what-is-daemon-thread-in-java-and.html 

# Threadpool : 
http://javarevisited.blogspot.in/2013/07/how-to-create-thread-pools-in-java-executors-framework-example-tutorial.html 

# Volatile keyword : 
http://javarevisited.blogspot.in/2011/06/volatile-keyword-java-example-tutorial.html 

# Interthread communication : 
http://javarevisited.blogspot.sg/2013/12/inter-thread-communication-in-java-wait-notify-example.html 

# Notify and notifyall :
http://javarevisited.blogspot.in/2012/10/difference-between-notify-and-notifyall-java-example.html 
If we don't call wait () or notify () method from synchronized context we will receive IllegalMonitorStateException in java. 

# Concurrent hash map : 
http://javarevisited.blogspot.sg/2011/04/difference-between-concurrenthashmap.html 

# Race condition : 
http://javarevisited.blogspot.sg/2012/02/what-is-race-condition-in.html 

# When to use thread and runnable : 
http://javarevisited.blogspot.sg/2012/01/difference-thread-vs-runnable-interface.html 

# Counting semaphore : 
http://javarevisited.blogspot.sg/2012/05/counting-semaphore-example-in-java-5.html 

# Countdown latches : 
http://javarevisited.blogspot.sg/2012/07/countdownlatch-example-in-java.html 

# Blocking queue : 
http://javarevisited.blogspot.in/2012/12/blocking-queue-in-java-example-ArrayBlockingQueue-LinkedBlockingQueue.html 

# Producer-consumer : 
http://javarevisited.blogspot.sg/2012/02/producer-consumer-design-pattern-with.html 

# Cyclic barrier : 
http://javarevisited.blogspot.in/2012/07/cyclicbarrier-example-java-5-concurrency-tutorial.html 

# Wait, notify, locking and synchronization interview questions : 
http://javarevisited.blogspot.in/2017/02/10-java-wait-notify-locking-and-synchronization-Interview-Questions-Answers.html#more 

# Reentrant lock : 
http://javarevisited.blogspot.in/2013/03/reentrantlock-example-in-java-synchronized-difference-vs-lock.html 
